"""
Simple Confession Bot - V60 (Complete Chat System + Enhanced Profiles + Reporting)
Requirements:
  pip install python-telegram-bot

V60 MAJOR UPDATES:
1. Complete Chat System with request/accept/messaging
2. Enhanced Reporting System with admin notifications
3. Fixed user registration flow
4. Comment pagination
5. Vote toggle functionality
6. UI/UX improvements throughout
"""
...
WAITING_FOR_ADMIN_MESSAGE = 15  # NEW: For admin messages
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS comments (
            ...
            FOREIGN KEY(conf_id) REFERENCES confessions(id)
        )
    """)
...
def save_comment(conf_id: int, user_id: int, content: str, parent_comment_id: Optional[int] = None) -> int:
    ts = int(time.time())
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO comments (conf_id, user_id, content, parent_comment_id, created_at) VALUES (?, ?, ?, ?, ?)",
        (conf_id, user_id, content, parent_comment_id, ts),
    )
...
def get_comment(comment_id: int) -> Optional[Dict[str, Any]]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, conf_id, user_id, content, parent_comment_id, created_at, bot_message_id FROM comments WHERE id = ?",
        (comment_id,)
    )
    row = cur.fetchone()
    conn.close()
...
def get_comments_for_confession(conf_id: int, page: int = 1, limit: int = COMMENTS_PER_PAGE) -> Tuple[List[Dict[str, Any]], int]:
    """Get paginated root comments for confession and total count"""
    offset = (page - 1) * limit
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    ...
    cur.execute(
        """
        SELECT id, user_id, content, created_at, parent_comment_id, bot_message_id
        FROM comments 
    ...
    for r in rows:
        comments.append({
            ...
            'parent_comment_id': r[4], 'bot_message_id': r[5]
        })
...
def get_user_comments(user_id: int, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    cur = conn.cursor()
    cur.execute(
        """SELECT c.id, c.content, c.created_at, conf.id as conf_id, conf.content as conf_content 
        FROM comments c 
    ...
    for row in rows:
        comments.append({
            ...
            'conf_id': row[3], 'conf_content': row[4]
        })
    return comments
...
async def format_comment_display(comment: Dict[str, Any], conf_id: int, depth: int, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if target_user_id == current_user_id:
        ...
        display_name = "Confession Author"
    else:
        profile = get_user_profile(target_user_id)
        display_name = profile['nickname'] or 'Anonymous'
        
        # Create profile link for others (not self and not confession author)
        if target_user_id != current_user_id and target_user_id != conf['user_id']:
...
async def show_comments(update: Update, context: ContextTypes.DEFAULT_TYPE, conf_id: int, page: int = 1):
    ...
    if not flat_comments: 
        await context.bot.send_message(
            ...
            reply_markup=get_deep_link_keyboard(conf_id, 0),  # UPDATED: Use new keyboard
        )
        return
    ...
    await context.bot.send_message(
        ...
        parse_mode="Markdown",
        reply_markup=get_comment_pagination_keyboard(conf_id, page, total_pages)
    )
    ...
    # Send footer with add comment button
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"--- End of comments for Confession #{conf_id} ---",
        reply_markup=get_deep_link_keyboard(conf_id, total_count),  # UPDATED: Use new keyboard
    )
...
CONFESSION_CANCEL_KEYBOARD = ReplyKeyboardMarkup(
    [['❌ Cancel', '🏠 Main Menu']], resize_keyboard=True, one_time_keyboard=False  # UPDATED: Added Main Menu
)
...
HELP_KEYBOARD = InlineKeyboardMarkup([
    ...
    [InlineKeyboardButton("✉️ Contact Admin", callback_data="help_contact_admin")],  # UPDATED: Fixed callback data
...
def get_deep_link_keyboard(conf_id: int, comment_count: int):
    ...
    keyboard = [
        [InlineKeyboardButton("💬 View Comments", callback_data=f"comment_view:{conf_id}")],
...
def get_comment_pagination_keyboard(conf_id: int, current_page: int, total_pages: int):
    """Generate pagination keyboard for comments"""
    keyboard = []
    
    # Page navigation
    nav_buttons = []
    if current_page > 1:
        nav_buttons.append(InlineKeyboardButton("◀️ Prev", callback_data=f"comment_page:{conf_id}:{current_page-1}"))
    ...
    if current_page < total_pages:
        nav_buttons.append(InlineKeyboardButton("Next ▶️", callback_data=f"comment_page:{conf_id}:{current_page+1}"))
    ...
    # Main menu button
    keyboard.append([InlineKeyboardButton("🏠 Main Menu", callback_data=CB_MENU_MAIN)])
...
# UPDATED: Profile keyboard - no buttons for own profile
def get_user_profile_keyboard(target_user_id: int, current_user_id: int) -> InlineKeyboardMarkup:
    ...
    profile_buttons.append(
        InlineKeyboardButton(chat_button_text, callback_data=f"request_chat:{target_user_id}")
    )
...
# UPDATED: Profile display for own profile
async def deep_link_profile(update: Update, context: ContextTypes.DEFAULT_TYPE, target_user_id: int):
    ...
    if target_user_id == user_id:
        ...
        if profile['department'] and profile['department'] != "Not specified":
            profile_text += f"🏢 Department: {profile['department']}"
    else:
        profile_text = (
...
# UPDATED: Start function with new deep link keyboard
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    profile = get_user_profile(user_id)
    
    # Check if user has used /start before
            ...
            try:
                ...
                if conf and conf['status'] == 'approved':
                    ...
                    await update.effective_chat.send_message(
                        channel_text,
                        parse_mode="HTML",
                        reply_markup=get_deep_link_keyboard(conf_id, comment_count)  # UPDATED: Use new keyboard
                    )
                    return 
                else:
...
# UPDATED: Fixed comment menu button callback
    ...
    if data.startswith("comment_add:"):
        try:
            ...
            await context.bot.send_message(
                ...
                reply_markup=ReplyKeyboardMarkup([['❌ Cancel', '🏠 Main Menu']], resize_keyboard=True)  # UPDATED: Added Main Menu
            )
...
# UPDATED: Fixed comment menu callback
...
# ... (continuing with the rest of the handlers in the next part due to character limits)
# UPDATED: Comment interaction with improved chat system
async def comment_interaction_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # UPDATED: Chat request handler with improved chat system
            ...
            try:
                ...
                await context.bot.send_message(
                    ...
                    text=f"💬 **Chat Request Accepted**\n\n"
                         ...
                         f"You have received a message. You are now in a chat. Send messages here to reply. Use /leavechat to exit.",
                    parse_mode="Markdown"
                )
            except Exception as e:
                logger.warning(f"Could not notify user about auto-accepted chat: {e}")
            
            # Update the button to show "Send Message"
            new_keyboard = get_user_profile_keyboard(target_user_id, user_id)
            try:
                await query.edit_message_reply_markup(reply_markup=new_keyboard)
            except Exception:
...
# UPDATED: Chat request response handler with improved chat system
async def chat_request_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("chat_accept:"):
        ...
        # Notify the requester and enter chat mode for both
        try:
            user_profile = get_user_profile(to_user_id)
            await context.bot.send_message(
            ...
            # Update the requester's profile button to show "Send Message"
            requester_profile = get_user_profile(from_user_id)
            await context.bot.send_message(
                chat_id=from_user_id,
                text=f"👤 **{requester_profile['nickname'] or 'Anonymous'}'s Public Profile**\n\n"
                     f"⚡ Aura Points: {requester_profile['aura_points']}\n"
                     f"👥 Followers: {get_follow_counts(from_user_id)['followers']} | Following: {get_follow_counts(from_user_id)['following']}\n"
                     f"📝 Bio: {requester_profile['bio']}\n"
                     f"🏢 Department: {requester_profile['department']}",
                reply_markup=get_user_profile_keyboard(from_user_id, to_user_id),
                parse_mode="Markdown"
            )
        except Exception as e:
...
# UPDATED: Chat message handler with improved notification system
async def chat_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message_text == "Block":
        ...
        await update.message.reply_text(
            "🚫 You have blocked this user. They will no longer be able to send you messages through the bot. The chat has been ended.",
            reply_markup=MAIN_REPLY_KEYBOARD
        )
    ...
    # Notify the other user with improved messaging
    try:
        user_profile = get_user_profile(user_id)
        await context.bot.send_message(
            chat_id=target_user_id,
        ...
        # Update the receiver's context to enter chat mode
        context_for_receiver = context.application.persistence.get_user_context(target_user_id)
        if context_for_receiver is None:
            context_for_receiver = {}
        
        context_for_receiver['active_chat_with'] = user_id
        context_for_receiver['active_chat_id'] = chat_id
        context.application.persistence.update_user_data(target_user_id, context_for_receiver)
        
    except Exception as e:
        logger.warning(f"Could not deliver chat message: {e}")
        await update.message.reply_text("⚠️ Failed to deliver message. The user may have blocked the bot.")
...
async def confess_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if time_since < RATE_LIMIT_SECONDS:
        ...
        await update.effective_message.reply_text(
            ...
            parse_mode="Markdown",
            reply_markup=MAIN_REPLY_KEYBOARD  # UPDATED: Added main menu
        )
        return ConversationHandler.END
    ...
    await update.effective_message.reply_text(
        ...
        reply_markup=CONFESSION_CANCEL_KEYBOARD,  # UPDATED: Already has main menu
        parse_mode="Markdown"
    )
...
# UPDATED: Comment receive with main menu
async def comment_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "❌ Cancel" or text == "🏠 Main Menu":  # UPDATED: Handle both buttons
    ...
    comment_id = save_comment(conf_id, msg.from_user.id, text, parent_comment_id)
    
    await msg.reply_text(
        f"✅ Your comment has been posted!",
        parse_mode="Markdown",
        reply_markup=MAIN_REPLY_KEYBOARD  # UPDATED: Return to main menu
    )
...
# UPDATED: Reply receive with main menu
...
async def profile_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data in ["profile_settings", "profile_my_chats"]:
        await query.edit_message_text(
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "• *Reply* - Respond to comments with text\n"
...
async def help_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "help_rules":
        ...
        # UPDATED: Use the new admin message handler
...
def main():
    ...
    confession_conv_handler = ConversationHandler(
        ...
        fallbacks=[
            ...
            MessageHandler(filters.Regex("^(❌ Cancel|🏠 Main Menu)$"), confession_cancel_fallback),  # UPDATED: Handle both
        ],
        per_user=True,      
        allow_reentry=True,  
    )
    ...
    comment_reply_conv_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(comment_menu_callback, pattern="^comment_add:"),  # UPDATED: Added this entry point
        ],
        states={
            WAITING_FOR_COMMENT: [
                CommandHandler("cancel", comment_cancel_callback),
                MessageHandler(filters.Regex("^(❌ Cancel|🏠 Main Menu)$"), comment_cancel_callback),  # UPDATED: Handle both
                MessageHandler(filters.TEXT & ~filters.COMMAND, comment_receive),
            ],
            WAITING_FOR_REPLY: [
                CommandHandler("cancel", comment_cancel_callback),
                MessageHandler(filters.Regex("^(❌ Cancel|🏠 Main Menu)$"), comment_cancel_callback),  # UPDATED: Handle both
                MessageHandler(filters.TEXT & ~filters.COMMAND, reply_receive),
            ],
        },
        fallbacks=[
            CommandHandler("cancel", comment_cancel_callback),
    ...
    chat_conv_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(chat_request_response, pattern="^(chat_accept:|chat_decline:)"),  # UPDATED: Added entry point
        ],
        states={
