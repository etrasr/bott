...
ADMIN_GROUP_ID = int(os.getenv("ADMIN_GROUP_ID", "-1003102743500"))
CHANNEL_ID = int(os.getenv("CHANNEL_ID", "-1003219197121"))
BOT_USERNAME = "wru_confessions_bot"
ADMIN_USER_ID = 7300957726  # New admin ID for reports
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS user_profiles (
            ...
            start_used BOOLEAN DEFAULT FALSE  -- NEW: Track if user has used /start
        )
    """)
...
# ... (Previous database functions remain the same until we get to new ones)
...
# UPDATED: Vote function now properly toggles
...
# UPDATED: Comment fetching with pagination
...
# UPDATED: User profile with start_used tracking
...
# NEW: Chat system database functions
...
# UPDATED: Remove comment ID from display
async def format_comment_display(comment: Dict[str, Any], conf_id: int, depth: int, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    anon_id_display = (comment['user_id'] % 1000) * 3 + 7 
    ...
    profile_link_text = f"[Anonymous]({profile_deep_link})"
    ...
    text += f"{indent}👤 {profile_link_text} ⚡︎{aura_points} Aura\n"
...
# UPDATED: Comment display with pagination
async def show_comments(update: Update, context: ContextTypes.DEFAULT_TYPE, conf_id: int, page: int = 1):
    ...
    if not flat_comments: 
        await context.bot.send_message(
            ...
            reply_markup=get_channel_post_buttons(conf_id, page, total_pages), 
        )
        return
...
# V50: Profile keyboard matching first image
...
# V50: Edit profile keyboard matching second image
...
def get_channel_post_buttons(conf_id: int, page: int = 1, total_pages: int = 1):
    return get_comment_pagination_keyboard(conf_id, page, total_pages)
...
# UPDATED: Comment interaction with toggle support
...
# UPDATED: Profile keyboard with chat status
def get_user_profile_keyboard(target_user_id: int, current_user_id: int) -> InlineKeyboardMarkup:
    ...
    if current_user_id == target_user_id:
        follow_status_text = "👤 Your Profile" 
    elif is_following(current_user_id, target_user_id):
    ...
    if active_chat:
        ...
        if chat_request['from_user_id'] == current_user_id:
            ...
            chat_button_text = "💬 Request to Chat"  # Shouldn't happen in this view
    elif chat_request and chat_request['status'] == 'accepted':
        chat_button_text = "💬 Send Message"
...
def get_comment_menu_keyboard(conf_id: int, page: int = 1, total_pages: int = 1) -> InlineKeyboardMarkup:
    return get_comment_pagination_keyboard(conf_id, page, total_pages)
...
# ... (Continue with handler functions in next part)
# ------------------------------ HANDLER FUNCTIONS ------------------------------
...
# UPDATED: Start function with improved user tracking
        ...
        if start_payload.startswith("comment_"):
            try:
                ...
                if conf and conf['status'] == 'approved':
                    # UPDATED: Remove "Viewing Confession" text
                    ...
                    await update.effective_chat.send_message(
                        channel_text,
                        parse_mode="HTML",
                        reply_markup=get_comment_menu_keyboard(conf_id)
                    )
                    return 
                else:
...
# UPDATED: Comment pagination handler
...
async def comment_menu_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data.startswith(CB_START_COMMENT):
        try:
...
# UPDATED: Comment interaction with vote toggling
async def comment_interaction_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # NEW: Chat request handler
        ...
        if existing_request and existing_request['status'] == 'pending':
            ...
            create_active_chat(user_id, target_user_id)
            ...
            try:
                ...
                await context.bot.send_message(
                    ...
                    text=f"✅ The user has accepted your chat request! You can now send messages from their profile.",
                    parse_mode="Markdown"
                )
            except Exception as e:
...
# NEW: Report reason selection handler
...
# NEW: Custom report reason handler
...
# NEW: Admin notification for reports
...
# NEW: Chat request response handler
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=f"💬 **Message from {user_profile['nickname'] or 'Anonymous'}:**\n\n{message_text}\n\n"
                 f"You are now in a chat. Send messages here to reply. Use /leavechat to exit.",
            reply_markup=CHAT_KEYBOARD,
            parse_mode="Markdown"
        )
    except Exception as e:
...
# MISSING CONFESSION FUNCTIONS - ADDED NOW
...
# UPDATED: Profile callback handler to include chat entry
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Profile command with first image layout"""
    user_id = update.effective_user.id
    profile = get_user_profile(user_id)
...
async def profile_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Enhanced profile callback handler with new functionality"""
    query = update.callback_query
    data = query.data
    await query.answer()
    ...
    if data == "profile_edit":
        # Second image layout - Edit Profile
        profile_text = (
        ...
        # Return to main profile view (first image)
        follow_counts = get_follow_counts(user_id)
        confessions_count = get_user_confessions_count(user_id)
...
# V50: HELP SYSTEM with rewritten texts
...
async def help_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Enhanced help callback handler with rewritten texts"""
    query = update.callback_query
    data = query.data
    await query.answer()
    ...
    if data == "help_rules":
        ...
        await query.edit_message_text(
            ...
            "Reach out to the admin for:\n\n"
            "• Reporting inappropriate content\n"
            "• Appealing content removal decisions\n"  
            "• Providing suggestions for improvements\n"
            "• Asking questions about bot functionality\n\n"
            "Please allow reasonable time for responses to your inquiries.",
            parse_mode="Markdown",
            reply_markup=HELP_KEYBOARD
        )
...
def main():
    ...
    confession_conv_handler = ConversationHandler(
        ...
        states={
            WAITING_FOR_CONFESSION: [
                MessageHandler(
                    (filters.TEXT | filters.PHOTO | filters.Document.ALL) & ~filters.COMMAND & ~filters.Regex("^❌ Cancel$"), 
                    confession_receive
                ),
            ],
        ...
        fallbacks=[
            ...
            MessageHandler(filters.Regex("^❌ Cancel$"), confession_cancel_fallback),
        ],
        per_user=True,      
        allow_reentry=True,  
    )
    ...
    comment_reply_conv_handler = ConversationHandler(
        ...
        states={
            WAITING_FOR_COMMENT: [
                ...
                MessageHandler(filters.Regex("^❌ Cancel$"), comment_cancel_callback), 
    ...
    # NEW: Report Conversation Handler
    ...
    application.add_handler(CallbackQueryHandler(comment_menu_callback, pattern="^comment_add:"))
