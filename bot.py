...
# UPDATED: Improved comment display with confession author identification and PROFILE LINKS
...
# UPDATED: Show comments with improved pagination at bottom
...
# UPDATED: Start function with proper user tracking
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    profile = get_user_profile(user_id)
    
    # Check if user has used /start before - FIXED: Proper persistence
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=f"👤 **{requester_profile['nickname'] or 'Anonymous'}'s Public Profile**\n\n"
                         ...
                         f"Department: {requester_profile['department'] or 'Not specified'}\n"
...
# FIXED: Chat message handler - removed duplicate error message
async def chat_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Notify the other user - FIXED: Only show error if delivery actually fails
    try:
        user_profile = get_user_profile(user_id)
        await context.bot.send_message(
            chat_id=target_user_id,
        ...
        # Only log the error, don't show duplicate message to sender
        logger.warning(f"Could not deliver chat message: {e}")
        # Don't show error message to sender if message was successfully saved
