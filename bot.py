...
# UPDATED: Profile keyboard with proper chat button states
def get_user_profile_keyboard(target_user_id: int, current_user_id: int) -> InlineKeyboardMarkup:
    ...
    # Chat request status - FIXED: Always show "Send Message" if active chat exists
...
def backup_database_to_dropbox():
    """Backup SQLite database to Dropbox"""
    try:
...
def restore_database_from_dropbox():
    """Restore SQLite database from Dropbox backup"""
    try:
        dbx = get_dropbox_client()
        if not dbx:
            return False
        ...
        # Download the file
        metadata, response = dbx.files_download("/confessions_backup.db")
        
        # Write to database file
        with open(DB_PATH, 'wb') as db_file:
            db_file.write(response.content)
        
        logger.info("Successfully restored database from Dropbox backup")
        return True
...
async def comment_interaction_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # UPDATED: Chat request handler with improved chat system
        ...
        # NEW: Start chat from deep link or My Chats
        target_user_id = int(data.split(":")[1])
...
# UPDATED: Chat request response handler with proper button updates
async def chat_request_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("chat_accept:"):
        ...
        # Notify the requester and update their profile button
        try:
