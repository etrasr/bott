"""
Simple Confession Bot - V62 (Fixed Chat System + Media Comments + Comment Order Fix + Enhanced Backup)
Requirements:
...
6. Improved chat request flow with proper button updates
"""
...
def backup_database_to_dropbox():
    ...
    try:
        # FIX: Check if database file exists first
        if not os.path.exists(DB_PATH):
            logger.info("No database file found to backup. Database may not be created yet.")
            return False
            
        # FIX: Check if database file is not empty
        if os.path.getsize(DB_PATH) == 0:
            logger.info("Database file is empty. Skipping backup.")
            return False
...
def backup_on_startup():
    ...
    if not os.path.exists(DB_PATH) or (os.path.exists(DB_PATH) and os.path.getsize(DB_PATH) == 0):
    ...
    # Then backup the current state - ONLY if database exists and has data
    if os.path.exists(DB_PATH) and os.path.getsize(DB_PATH) > 0:
        ...
        logger.info("💡 No database to backup on startup - will backup after first confession")

def trigger_immediate_backup():
    """Manually trigger a backup - for testing and after important actions"""
    logger.info("🔄 Manually triggering immediate backup...")
    if backup_database_to_dropbox():
...
async def test_backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    ADMIN_ID = 7300957726  # Your admin ID
    ...
    if trigger_immediate_backup():
        await update.message.reply_text("✅ Backup test successful! Check logs for details.")
    else:
        await update.message.reply_text("❌ Backup test failed! Check logs for errors.")
...
def main():
    ...
    # Other Handlers - IMPORTANT: /forcerestore placed BEFORE unknown handler
