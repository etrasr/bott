...
ADMIN_GROUP_ID = -1003131561656  # CHANGED: New admin group ID
...
# UPDATED: Profile keyboard - no buttons for own profile, improved chat system
...
async def chat_request_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("chat_accept:"):
        ...
        try:
            ...
            new_keyboard = get_user_profile_keyboard(from_user_id, to_user_id)
            try:
                await context.bot.send_message(
                    chat_id=to_user_id,
                ...
                logger.warning(f"Could not update requester's profile: {e}")
...
async def chat_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message_text == "Block":
        ...
        await update.message.reply_text(
            f"🚫 You have blocked {get_user_profile(target_user_id)['nickname'] or 'Anonymous'}. They will no longer be able to send you messages through the bot. The chat has been ended.",
            reply_markup=MAIN_REPLY_KEYBOARD
        )
