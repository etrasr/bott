...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Help command matching the exact text format from the first picture"""
    help_text = (
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Enhanced error handler with logging and backup protection"""
    try:
        ...
        if update and update.effective_chat:
            try:
                error_message = (
                    ...
                    "• Contact admin if problem persists\n\n"
                    "🔄 *Backup System:* Your data is safe and backed up."
                )
        ...
        # Enhanced backup protection - trigger backup on critical errors
        if context.error and isinstance(context.error, (sqlite3.Error, RuntimeError)):
            logger.warning("🔄 Database-related error detected, triggering emergency backup...")
            enhanced_backup_trigger()
            
    except Exception as e:
        # If the error handler itself fails
        logger.critical(f"CRITICAL: Error handler failed: {e}")
        
    finally:
        # Always log that we handled the error
...
def main():
    ...
    # Add post_init to start backup monitor after app is running
    async def post_init(application):
        # Start backup monitor when application is running
